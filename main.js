   ...
   if (pairingCode && !XeonBotInc.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Start with country code of your WhatsApp Number, Example : +22781289418")))
            process.exit(0)
         }
      } else {
         phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Please type your WhatsApp number 😍\nFor example: +22781289418 : `)))
         phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            ...
            phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`TST-BOT Please type your WhatsApp number 😍\nFor example: +22781289418 : `)))
            phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
            rl.close()
         }
      }
...
XeonBotInc.ev.on("connection.update",async  (s) => {
        ...
        if (connection == "open") {
			...
			await delay(1999)
            console.log(chalk.yellow(`\n\n                  ${chalk.bold.blue(`[ TST-BOT ]`)}\n\n`))
            console.log(chalk.cyan(`< =============K7=========TST-BOT================= >`))
	        console.log(chalk.magenta(`\n${themeemoji} YT CHANNEL: KA-WA-KI`))
            console.log(chalk.magenta(`${themeemoji} GITHUB: 0-SOUCIS `))
            console.log(chalk.magenta(`${themeemoji} INSTAGRAM: @kawaki2279 `))
            console.log(chalk.magenta(`${themeemoji} WA NUMBER: 22781289418`))
            console.log(chalk.magenta(`${themeemoji} CREDIT: KAWAKI227\n`))
        }
        if (
            connection === "close" &&
            lastDisconnect &&
